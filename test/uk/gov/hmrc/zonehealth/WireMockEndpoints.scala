...
trait WireMockEndpoints extends Suite with BeforeAndAfterAll with BeforeAndAfterEach {
  ...
  def zoneHealthProtectedEndpointResponse(willRespondWith: (Int, Option[String])) =
    registerEndpoint(RequestMethod.GET, "/zone-health", Map(), willRespondWith)

  def registerEndpoint(method:RequestMethod, url:String,
                       extraHeaders:Map[String,String] = Map(), willRespondWith: (Int, Option[String])): Unit = {
    val builder = new MappingBuilder(method, urlEqualTo(url))
    val response: ResponseDefinitionBuilder = new ResponseDefinitionBuilder().withStatus(willRespondWith._1)
